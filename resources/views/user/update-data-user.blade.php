...
@section('container')
	<section class="pt-4">
		...
		<form id="form-data-pemagang" class="mt-5" method="post" action="{{ route('update.data.put', $data->id) }}"
			...
			<div class="mb-4">
				...
				<input type="text" class="form-control @error('name') is-invalid @enderror" id="name" name="name"
					placeholder="Masukkan nama lengkap" value="{{ $data->name }}" required />
				@error('name')
			...
			<div class="mb-4">
				...
				<input type="number" class="form-control @error('student_id') is-invalid @enderror" id="student_id"
					name="student_id" placeholder="Masukkan ID pelajar" value="{{ $data->student_id }}" required />
				@error('student_id')
					<div class="invalid-feedback my-1">
						{{ $message }}
					</div>
			...
			<div class="mb-4">
				...
				<input type="text" class="form-control @error('school') is-invalid @enderror" id="school" name="school"
					placeholder="Masukkan asal sekolah/universitas" value="{{ $data->school }}" required />
				@error('school')
					<div class="invalid-feedback my-1">
						{{ $message }}
					</div>
			...
			<button type="submit" class="btn btn-primary mt-2" style="">Perbarui Data</button>
		</form>
	...
	<script>
		...
		document.addEventListener("DOMContentLoaded", function() {
			...
			formPemagang.addEventListener("submit", function(event) {
				...
				Swal.fire({
					title: "Perbarui Data",
					text: "Pastikan data pribadi yang Anda masukkan sudah benar.",
					icon: "warning",
